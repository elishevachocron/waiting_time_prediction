...
for experiment in [10]:
    print('--------------------------------------Experiment ' + str(experiment)+'--------------------------------------')
    path_to_load = path + str(experiment)
    path_to_save = path + str(experiment) + '\\Results_new_tuning'
    for key_type, features_list in features_type.items():
        if key_type == 'set_features_1':
            continue
        ...
        data = data.replace([np.inf, -np.inf], np.nan).dropna(subset=['Checkin_time'], axis=0, how="any")
    ...
    # ---------------Test/Train Split----------------
        ...
        y_QTD_test = df.loc[(df.Week > week_split) & ~(df.Checkin_time.isna())]['WT_QTD_bis'].to_frame()
        y_QTD_test.reset_index(drop=True, inplace=True)

        y_LES_test = df.loc[(df.Week > week_split) & ~(df.Checkin_time.isna())]['LES'].to_frame()
        y_LES_test.reset_index(drop=True, inplace=True)
        # TODO check if ther is any missing value (y_LES_test.isna().sum())
        y_HOL_test = df.loc[(df.Week > week_split) & ~(df.Checkin_time.isna())]['HOL'].to_frame()
        ...
        #X_test = df.loc[df.Week > week_split][final_features_list]
        X_test = df.loc[(df.Week > week_split) & ~(df.Checkin_time.isna())][final_features_list]
        X_test.reset_index(drop=True, inplace=True)
        y_train = df.loc[df.Week <= week_split]['Real_WT'].to_frame()
        #y_test = df.loc[df.Week > week_split]['Real_WT'].to_frame()
        y_test = df.loc[(df.Week > week_split) & ~(df.Checkin_time.isna())]['Real_WT'].to_frame()
        ...
        for algo in algorithms_used:
            if algo == 'RF':
                if model_state[algo] == 'fit':
                    ...
                    with open(path_to_save + '/RF_3'+key_type+'.pkl', 'wb') as file:  # save the trained model
                ...
                y_pred_rf_train = grid_rf.best_estimator_.obb_prediction_
                ...
                cv_train = rmse_train / mean_WT_train
                cv_test = rmse_test/mean_WT_test
                bias = np.mean([y_test.to_numpy()[i] - y_pred_rf_test[i] for i in range(len(y_test))])*3600
                results_dict['RF'] = [grid_rf.best_estimator_, round(rmse_train, 2), round(rmse_test, 2), round(cv_train, 2),
                                      round(cv_test, 2), round(mae, 2), round(re, 2), round(bias, 2)]
                plt.figure()
                ...
                if model_state[algo] == 'fit':
                    ...
                    with open(path_to_save + '/SVR_3'+key_type+'.pkl', 'wb') as file:  # save the trained model
                ...
                results_dict['SVR'] = [grid_svr.best_estimator_, round(rmse_train, 2), round(rmse_test, 2), round(cv_train, 2),
                                      round(cv_test, 2), round(mae, 2), round(re, 2), round(bias, 2)]
                ...
                if model_state[algo] == 'fit':
                    ...
                    with open(path_to_save + '/GBR_3'+key_type+'.pkl', 'wb') as file:  # save the trained model
                ...
                results_dict['GBR'] = [grid_gbr.best_estimator_, round(rmse_train, 2), round(rmse_test, 2), round(cv_train, 2),
                                      round(cv_test, 2), round(mae, 2), round(re, 2), round(bias, 2)]
                plt.figure()
                ...
                if model_state[algo] == 'fit':
                    ...
                    with open(path_to_save + '/ANN_3'+key_type+'.pkl', 'wb') as file:  # save the trained model
                ...
                results_dict['ANN'] = [grid_ann.best_estimator_, round(rmse_train, 2), round(rmse_test, 2), round(cv_train, 2),
                                      round(cv_test, 2), round(mae, 2), round(re, 2), round(bias, 2)]
            elif algo == 'LR':
                print('---------------LR FITTING---------------')
                ...
                results_dict['LR'] = [reg.coef_, round(rmse_train, 2), round(rmse_test, 2), round(cv_train, 2),
                                      round(cv_test, 2), round(mae, 2), round(re, 2), round(bias, 2)]
                pass
            elif algo == 'QTD':
        ...
        cv_train_metrics = {'LR': [], 'RF': [], 'SVR': [], 'GBR': [], 'ANN': [], 'QTD': [], 'LES': [], 'HOL': []}
        cv_test_metrics = {'LR': [], 'RF': [], 'SVR': [], 'GBR': [], 'ANN': [], 'QTD': [], 'LES': [], 'HOL': []}
        ...
        df_results = pd.DataFrame(columns=['RMSE_train', 'RMSE_test', 'CV_train', 'CV_test', 'MAE', 'RE', 'Bias', 'HyperParameters'],
        ...
        for algo, results in results_dict.items():
            ...
            cv_train_metrics[algo].append(results[3])
            cv_test_metrics[algo].append(results[4])
            mae_metrics[algo].append(results[5])
            re_metrics[algo].append(results[6])
            bias_metrics[algo].append(results[7])
        ...
        df_results['CV_train'] = np.concatenate(list(cv_train_metrics.values()))
        df_results['CV_test'] = np.concatenate(list(cv_test_metrics.values()))
        ...
        df_results.to_csv(path_to_save+'\\Abandonment_indicator/Results_3_'+key_type+'.csv')
